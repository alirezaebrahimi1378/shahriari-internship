...
class process:
    def __init__(self, *, dictionary: dict, label: dict):
        ...
        # create list of labels from the GT_2's value that use in confusion
        # matrix
    ...
    # find IDE by variance from PCA feature extraction that apply on sentinel
    # images
    def ide(self) -> int:
        """_find IDE_
    ...
    # apply pca on sentinel data that number of components = ide and save it
    # to dictionary
    def pca(self) -> dict:
    ...
    def KNN(self):
        ...
        print('number of class in predicted data by knn ',
              np.unique(data_predict, return_counts=True))
        ...
        for i in range(4):
            ...
            if i == 3:
                ...
                patches = [mpatches.Patch(
                    color=colors[i], label=self.label[values[i]]) for i in range(len(values))]
...
filenames = [
    'GT_1.tif',
    'GT_2.tif',
    'sentinel-2_part1.tif',
    'sentinel-2_part2.tif']
